...
2. The page builds an `geminiMessageFormat.toApi(messages)` payload and `POST`s to `/api/chat`.
...
- Node.js 18+
- pnpm
- An Gemini AI API key

### 1. Install dependencies

```bash
cd examples/form-generator-gemini
pnpm install
```

### 2. Configure environment

Create a `.env.local` file in the `examples/form-generator-gemini/` directory:
